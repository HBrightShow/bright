...
namespace eosiosystem {
   ...
   void validate_b1_vesting( int64_t stake ) {
      ...
      const int64_t max_claimable = 100'000'0000000ll;
...
#if DEBUG_DELEGATE
	 print("from:", from, "receiver:", receiver, "stake_net_delta:", stake_net_delta.amount, 
	 "stake_cpu_delta:", stake_cpu_delta.amount, "transfer:", transfer, "\n");
#endif
	//End: add by UOS(hml) Date:2018-09-26
      ...
      // update voting power
      {
      //Begin: modify by UOS(hml) Date:2018-09-22
	  /*
		  1.the fee of transaction wil be pay for stake_net_delta, no longer used to vote producer.
		  2.stake_cpu_delta will be used to vote producers.

	  */	  
         //asset total_update = stake_net_delta + stake_cpu_delta;
	  	 asset total_update = stake_cpu_delta;
	  //End: modify by UOS(hml) Date:2018-09-22
         ...
         if( from_voter->vote_producers.size() > 0 ) {
            validate_votes( from, total_update);
         }
