...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "UOS") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
...
//Begin: add by UOS(hml) Date:2018-09-26

inline std::string& int64_to_double_string(const int64_t amount, std::string& result) {
	result.clear();
...
//End: add by UOS(hml) Date:2018-09-26
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         createAccount->add_option("--stake-net", stake_net,
                                   (localized("The amount of UOS delegated for net bandwidth")))->required();
         createAccount->add_option("--stake-cpu", stake_cpu,
                                   (localized("The amount of UOS delegated for CPU bandwidth")))->required();
         ...
         createAccount->add_option("--buy-ram", buy_ram_eos,
                                   (localized("The amount of RAM bytes to purchase for the new account in UOS")));
         createAccount->add_flag("--transfer", transfer,
                                 (localized("Transfer voting power and right to unstake UOS to receiver")));
      }
...
struct vote_producers_subcommand {
   ...
   vote_producers_subcommand(CLI::App* actionRoot) {
	  ...
	  vote_producers->add_option("quantity", quantity, localized("The voting quantity"))->required();
...
struct cancel_vote_producer_subcommand {
   string voter_str;
   string	quantity;	
   vector<eosio::name> producer_names;

   cancel_vote_producer_subcommand(CLI::App* actionRoot) {
      auto cancel_vote_producers = actionRoot->add_subcommand("cancelvote", localized("Cancle vote for one or more producers"));
      cancel_vote_producers->add_option("voter", voter_str, localized("The cancel voting account"))->required();
	  cancel_vote_producers->add_option("quantity", quantity, localized("The cancel voting quantity"))->required();
      cancel_vote_producers->add_option("producers", producer_names, localized("The account(s) to cancel vote for. All options from this position and following will be treated as the producer list."))->required();
      add_standard_transaction_options(cancel_vote_producers);

      cancel_vote_producers->set_callback([this] {
         ...
         fc::variant act_payload = fc::mutable_variant_object()
                  ...
                  ("quantity", to_asset(quantity))
                  ("producers", producer_names);
         send_actions({create_action({permission_level{voter_str,config::active_name}}, config::system_account_name, N(cancelvote), act_payload)});
      });
   }
};
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
		 ...
		 //Begin: modify by UOS(hml) Date:2018-09-27
		...
		//End: modify by UOS(hml) Date:2018-09-27
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_net_quantity", stake_net_amount, localized("The amount of UOS to stake for network bandwidth"))->required();
      delegate_bandwidth->add_option("stake_cpu_quantity", stake_cpu_amount, localized("The amount of UOS to stake for CPU bandwidth"))->required();
      delegate_bandwidth->add_option("--buyram", buy_ram_amount, localized("The amount of UOS to buyram"));
      delegate_bandwidth->add_flag("--transfer", transfer, localized("Transfer voting power and right to unstake UOS to receiver"));
...
struct undelegate_bandwidth_subcommand {
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->add_option("unstake_net_quantity", unstake_net_amount, localized("The amount of UOS to undelegate for network bandwidth"))->required();
      undelegate_bandwidth->add_option("unstake_cpu_quantity", unstake_cpu_amount, localized("The amount of UOS to undelegate for CPU bandwidth"))->required();
...
struct bidname_subcommand {
   ...
   bidname_subcommand(CLI::App *actionRoot) {
      ...
      bidname->add_option("bid", bid_amount, localized("The amount of UOS to bid"))->required();
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("amount", amount, localized("The amount of UOS to pay for RAM, or number of kbytes of RAM if --kbytes is set"))->required();
...
struct sellram_subcommand {
   ...
   sellram_subcommand(CLI::App* actionRoot) {
      ...
      sellram->add_option("account", receiver_str, localized("The account to receive UOS for sold RAM"))->required();
...
void get_account( const string& accountName, bool json_format ) {
   ...
   if (!json_format) {
	  ...
	  //Begin: removed by UOS(hml) Date:2018-09-25
	  // here will rewrite the vote_info
#if 0
...
#endif
	//End: removed by UOS(hml) Date:2018-09-25
	//Begin: add by UOS(hml) Date:2018-09-25
	
	if ( res.voter_info.is_object() ) {
		
        auto& obj = res.voter_info.get_object();
#if 0
		for(variant_object::iterator iter = obj.begin(); iter != obj.end(); ++iter) {
			std::cout << iter->key() << std::endl;
			if(iter->key() == "vote_producers") {
			}
		}
#endif	
		...
		std::string strTemp;
		int64_t staked_cpu = obj["staked"].as_int64();

		//staked(staked_cpu);
		int64_to_double_string(staked_cpu, strTemp);

		std::cout << indent << std::left << std::setw(11)
			<< "staked:" << std::right << std::setw(18) << strTemp + " " + staked.symbol_name() << std::endl;

		fc::from_variant(obj["vote_producers"], voteproducer);

		int64_t totol_voted = 0;
		int64_t voted = 0;
		string strVoted;
        if ( voteproducer.size() > 0 ) {
           for ( auto vp : voteproducer ) {
		   	voted = vp.voted.get_amount();
		   	totol_voted += voted;
			int64_to_double_string(voted, strTemp);
		   
             std::cout << indent << std::left << std::setw(11)
			 	<< vp.producer.to_string() + ":" << std::right << std::setw(18)
			 	<< strTemp + " " + vp.voted.symbol_name() << std::endl;
           }
		...
		int64_to_double_string(totol_voted, strTemp);
		std::cout << indent << std::left << std::setw(11)
			<< "total:" << std::right << std::setw(18) << strTemp + " " + staked.symbol_name() << std::endl;
         
      }	

	  //End: add by UOS(hml) Date:2018-09-25
...
//                                ->check(CLI::ExistingFile);
   ...
   string amount;
   string memo;
   auto transfer = app.add_subcommand("transfer", localized("Transfer UOS from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending UOS"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving UOS"))->required();
   transfer->add_option("amount", amount, localized("The amount of UOS to send"))->required();
   ...
   //Begin: modify by UOS(hml) Date:2018-09-21
    auto cancelVoteProducer = cancel_vote_producer_subcommand(system);
   //End: modify by UOS(hml) Date:2018-09-21
